        ...
        UUID reversedTransactionId)
{
    public JournalTransaction
    {
        ...
        if (lines.size() < 2)
        {
            throw new IllegalArgumentException("journal transaction must contain at least two lines");
        }
        if (!isBalanced(lines))
        {
            throw new IllegalArgumentException("journal transaction lines must balance debits and credits");
        }
    }

    public static JournalTransaction create(String groupCode, LocalDate postedOn, String memo,
                                            TransactionTiming timing, List<PostingLine> lines)
    {
        return new JournalTransaction(UUID.randomUUID(), groupCode, postedOn, memo, timing, lines, null);
    }

    public JournalTransaction reverseOn(LocalDate reversalDate, String reversalMemo)
    {
