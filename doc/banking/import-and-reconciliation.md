...
Authorization does not weaken the existing source-hash, configured-account identity, duplicate, idempotency, profile uniqueness/ownership, acceptance, closed-period, finalized-reconciliation, balanced-transaction, or rollback rules. Existing actor strings also remain compatibility inputs until the separate authenticated-audit-actor tranche. Production current-session guard construction for all of these services remains owned by the later consolidated `UiServiceRegistry` wiring tranche.
