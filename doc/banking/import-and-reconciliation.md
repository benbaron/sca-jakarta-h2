...
Strict OFX/QFX preview and reviewed-row acceptance preview remain non-mutating. Mapped CSV continues to delegate its durable review commit through `BankStatementReviewService`. Direct normalized CSV review commit through `NormalizedBankCsvReviewService.commit(...)` likewise requires `BOOKKEEPING_WRITE`, while normalized CSV preview remains non-mutating. `BankImportReviewService.importForInterchange(...)` remains a caller-owned SCLX restoration seam and is intentionally not independently guarded because the outer SCLX commit owns authorization and atomicity.

Bank CSV mapping-profile maintenance is bookkeeping configuration for statement review. `BankCsvMappingProfileService.create(...)`, `replace(...)`, and `setActive(...)` therefore require `BOOKKEEPING_WRITE`; `list(...)` remains a read-only query. Authorization is checked before profile parsing or transaction work, so lower-privilege callers cannot use malformed input to enter a protected mutation path.

Authorization does not weaken the existing source-hash, configured-account identity, duplicate, idempotency, profile uniqueness/ownership, acceptance, closed-period, finalized-reconciliation, balanced-transaction, or rollback rules. Existing actor strings remain source-compatible inputs, but guarded production review, normalized-CSV, and reviewed-row acceptance writes derive the authoritative current-operation actor from the authenticated session. `UiServiceRegistry` supplies the current-session guard to these production services. Imported historical actor facts remain source history rather than current-operation identity. Governing detail: `doc/P20-S3-authenticated-audit-actor.md`.
