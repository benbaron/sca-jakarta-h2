...
Configured-bank-account/session listing, reconciliation snapshot/comparison review, and other read-only inspection remain readable to an authenticated VIEWER. Existing source-compatible unguarded constructors remain only as legacy/test composition seams; production `UiServiceRegistry` injects the current-session `AuthorizationGuard`. UI disabling remains explanatory and is not the security boundary.
...
Authorization does not replace reconciliation integrity rules. After permission succeeds, finalized sessions remain immutable, match/unmatch remains symmetric and company/configured-account scoped, cleared state remains attached to the canonical bank split, and successor history remains explicit. Successor audit facts use the authenticated username in guarded production composition rather than the command actor string. This changes no schema, migration, import parser, JavaFX control policy, or legacy reconciliation-run compatibility service.
