...
## P05-C8 canonical bank-ledger activity correction

P05-C8 changes the production **Bank Transactions** workspace so its first/default **Ledger Activity** tab is a canonical accounting view rather than an imported-statement list. `LedgerQueryService` reads `TxnSplit` rows whose transaction belongs to the active company and whose account is the `Account` linked by a `CompanyBankAccount`. The linked account must still satisfy `ASSET / BANK-function / DEBIT`; `CASH` is intentionally not required. Each visible row therefore represents the exact bank-account side of a canonical journal transaction and exposes the configured account, ledger account, fund, payee, memo, debit/credit presentation, and the split's authoritative cleared state/date.

The workspace keeps imported durable `BankStatementLine` facts under a separate **Statement Review** tab. Reviewed-row acceptance, statement-linked Journal drill-through, and statement export remain review/evidence operations and do not become the accounting ledger. Import alone does not create Ledger Activity; only a canonical `Txn` / `TxnSplit` does. Matching and reconciliation continue to mutate cleared state only through their existing governed services, while Ledger Activity remains read-only.

Configured-account identity is required for this view: a standalone BANK-function chart account that is not linked through `CompanyBankAccount` is not presented as configured-bank activity. Historical ledger activity for an inactive configured account remains viewable, while statement export continues to offer only active configured accounts. This preserves historical accounting visibility without reopening inactive accounts for new statement operations.

## P20-S3 reconciliation runtime authorization

The authoritative service-owned reconciliation mutations require `ApplicationPermission.BOOKKEEPING_WRITE`. `BankReconciliationWorkspaceService` enforces that permission before its interactive session-start, successor-start, manual statement-entry, auto-match, explicit match/unmatch, cleared-state, factual difference-explanation, Save, and Finalize operations. `BankClearedStateService.markMatchedAndCleared(...)` enforces the same permission before its direct reviewed-statement-to-ledger cleared-state mutation. The fixed P20 reserved-role policy therefore allows ADMIN, MANAGER, and ACCOUNTANT bookkeeping writes and denies VIEWER.

Configured-bank-account/session listing, reconciliation snapshot/comparison review, and other read-only inspection remain readable to an authenticated VIEWER. Existing source-compatible unguarded constructors remain only as legacy/test composition seams until the consolidated `UiServiceRegistry` wiring tranche injects the current-session `AuthorizationGuard` into production service construction; UI disabling remains explanatory and is not the security boundary.

Caller-owned interchange transaction seams remain deliberately unguarded at this inner layer. `BankReconciliationWorkspaceService.importForInterchange(...)` and `BankClearedStateService.applyForImport(...)` continue to require an active caller-owned `EntityManager` transaction, company ownership, and their existing integrity checks. The later import/SCLX commit service is the single outer authorization and atomicity owner for those paths, avoiding nested or competing authorization authorities.

Authorization does not replace reconciliation integrity rules. After permission succeeds, finalized sessions remain immutable, match/unmatch remains symmetric and company/configured-account scoped, cleared state remains attached to the canonical bank split, and successor history remains explicit. This tranche changes no schema, migration, import parser, JavaFX control policy, legacy reconciliation-run compatibility service, or authenticated audit-actor authority; those latter P20 items remain separately governed.
