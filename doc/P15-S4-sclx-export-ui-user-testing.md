...
Open a company with at least one fixed asset and one completed depreciation run. Export SCLX and confirm that the completion counts agree with the Asset Register and completed-run history. Verify that inactive or disposed assets needed for history are retained, each run points to its asset and canonical depreciation transaction, and mutable asset/account/fund names are not used as identity. A company with no fixed assets must contain `fixedAssets` with empty `assets` and `depreciationRuns` arrays rather than reporting the section as deferred. Switch companies and repeat to verify strict company isolation.
