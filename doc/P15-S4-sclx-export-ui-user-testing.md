...
The existing selected-company SCLX export action now includes `extensions.scaJakartaH2.fixedAssets` version 1 and no longer reports fixed assets as deferred. Create or open a company with at least one fixed asset and one completed depreciation run, export SCLX, and verify that the `assets` entry preserves the register fields and account/fund references and that the `depreciationRuns` entry references both the exported asset and the canonical transaction created by the run. Verify that an absent optional asset or run note is omitted rather than serialized as an empty value. Repeat with a company that has no fixed assets; the extension must still be present with empty arrays and must not produce a deferred warning. No new menu action is added.
