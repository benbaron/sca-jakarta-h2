...
| `JOURNAL_PANE` | `JournalPane` | filter fields, general-journal table, New/Edit Selected handoff | `TransactionEntryService.search` and `journalView` projections | navigation only; New/Edit opens Transaction Editor | yes for queried H2 data | yes; reads canonical `txn`/`txn_split` projections | canonical transaction service | no direct grid editing; supplemental records are provisioned by design but owned by future domain services | table-state preference hardening and supplemental record domain panels remain follow-ups | P03 |
