...
| `RECONCILIATION_RUNS` | `ReconciliationRunsPanel` | run table, record started/completed/failed buttons, comparison-workflow note | `ReconciliationService`/JDBC repository | reconciliation run repository | yes | partially; run records are H2 | JDBC reconciliation repository | no approval controls remain; still needs configured-account reconciliation comparison workflow | configured-account reconciliation comparison and unresolved-report save/edit workflow | P06 |
