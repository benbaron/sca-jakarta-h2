...
Status: P00 inventory of current main, updated through P09-S1 inventory persistence. This document records visible operations and data authority so later phases can replace placeholders without rescanning the whole UI.
...
| `TXN_EDITOR` | `TransactionEditorPanel` | header fields, split table, validate/save/run commands | lookup services and canonical transaction load/update routing | `TransactionEntryService` for New/Edit save paths | yes after save | yes | account/fund/budget lookups, canonical transaction service | unsaved drafts remain editor-local by design | Delete/correction affordance implemented in P03-C3; table-state preference hardening remains a follow-up | P02/P03 |
| `JOURNAL_PANE` | `JournalPane` | filter fields, sortable/resizable/reorderable general-journal table, supplemental-record provision area, New/Edit Selected handoff | `TransactionEntryService.search` and `journalView` projections | navigation only; New/Edit opens Transaction Editor | yes for queried H2 data; table state persists by active company key | yes; reads canonical `txn`/`txn_split` projections | canonical transaction service | no direct grid editing; supplemental records are provisioned by design but owned by future domain services | full company money/date preference model remains a cross-panel follow-up; supplemental domain panels remain later slices | P03 |
...
| `INVENTORY` | `InventoryPanel` | inventory item table, movement-history table, item form, account/fund selectors, save/new, receipt/issue/adjustment buttons, disabled Delete explanation | `InventoryService`, account/fund lookup services | `InventoryService.create/update/recordMovement` | yes | yes | `inventory_item`, `inventory_movement`, chart accounts, funds | old inventory runbook removed | financially relevant movement-to-ledger automation, reports, and table-state polish | P09 |
