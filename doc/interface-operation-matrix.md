...
Status: P00 inventory of current main, updated through P03-C6 unified Journal workspace. This document records visible operations and data authority so later phases can replace placeholders without rescanning the whole UI.
...
| `JOURNAL_PANE` | `JournalWorkspacePanel` | one grouped transaction journal; filters; New Entry, Edit Selected, Save Entry, Delete/Reverse, Refresh; editable entry-line table; additional details; persisted supplemental-detail tabs; draggable resize bars between major sections | `TransactionEntryService.search/load`, transaction reference-data service | `TransactionEntryService.enter/update`; `TransactionCorrectionService.delete/reverse`; persisted `txn_supplemental_line` rows | yes; table and divider state also persist per active company | yes for transaction header, lines, and supplemental details | canonical `Txn`/`TxnSplit`, `txn_supplemental_line`, account/fund/budget/activity/merchant/counterparty reference choices | donor `Journal*` classes are design reference only; no donor persistence or alternate ledger is imported | desktop visual validation at laptop width; cleared display should later use an explicit line-level projection rather than the current aggregate label | P02/P03 |
