...
Status: P00 inventory of current main, updated through P09-C1 inventory UI rule corrections. This document records visible operations and data authority so later phases can replace placeholders without rescanning the whole UI.
...
| `INVENTORY` | `InventoryPanel` | inventory item table, movement-history table, item form, account/fund selectors, save/new, receipt/issue/adjustment buttons | `InventoryService`, account/fund lookup services | `InventoryService.create/update/recordMovement` | yes | yes | `inventory_item`, `inventory_movement`, chart accounts, funds | old inventory runbook removed; P09-C1 adds global New/Save, dirty state, table-state persistence, validation highlighting, and common money/date parsing/formatting | financially relevant movement-to-ledger automation and reports | P09 |
