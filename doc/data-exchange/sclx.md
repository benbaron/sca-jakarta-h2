...
`extensions.scaJakartaH2.bankStatementFacts` is an object with exactly four arrays: `importBatches`, `statementLines`, `issues`, and `transactionLineClearance`. Import batches preserve durable identity, optional configured-account reference, source filename and SHA-256 or equivalent persisted hash, source format, optional variant/version/encoding, source institution/bank/account/type, currency, statement dates and balances, configured-account match/confirmation facts, review status and timestamps, line/accept/reject/issue counts, and notes. Statement lines additionally preserve optional currency and correction action/reference facts. The source filesystem path and importing-user identity are explicitly excluded. Older SCLX documents that omit the P15-S6 metadata remain valid and import those fields as absent/false.
