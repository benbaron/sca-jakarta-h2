...
active_head: HEAD (Add drill-through debug logging)
...
- on 2026-07-06, `mvn -DskipTests compile` and focused `mvn -Dtest=DrillThroughCoordinatorTest,LedgerRegisterPanelTest test` passed after adding drill-through debug logging.
...
- **Open Selected in Editor** recalls the selected transaction into Transaction Editor and raises/focuses that workspace tab in the production shell; diagnostic stderr messages now show the context handoff stages for troubleshooting.
