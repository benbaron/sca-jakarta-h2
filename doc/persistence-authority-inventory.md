...
| Fixed assets/depreciation | `FixedAsset` and `FixedAssetDepreciationRun` JPA entities with V51 tables; depreciation runs create canonical `Txn` rows | yes for P08-S1 asset records and completed depreciation runs | old asset/depreciation text sidecars removed from production paths | later hardening: richer disposal/impairment workflows, visual polish, and reports |
...
5. V49 adds reconciliation protection, V50 adds budget plans/lines, and V51 adds fixed asset/depreciation tables. These migrations must remain nondestructive.
