...
Status: P00 inventory of current main, updated through active P16-S17 end-to-end closure. This document identifies duplicate authority risks, non-H2 stores, and migration hazards before later phases choose canonical models.
...
| Fixed assets/depreciation/lifecycle | `FixedAsset`, `FixedAssetDepreciationRun`, and `FixedAssetLifecycleEvent` JPA entities with V55/V71 tables; depreciation and confirmed Sale/Retirement/Impairment operations create linked canonical `Txn` rows | yes for asset records, completed depreciation runs, immutable lifecycle facts/status transitions, transaction/reversal links, portable identities, and factual audits | old asset/depreciation text sidecars are removed; generic Journal mutation is blocked for lifecycle-linked transactions so asset state cannot diverge from the ledger | retain the P16-S14 domain reversal boundary; add specialized reports in P16-S15; version SCLX separately only with semantic round-trip coverage |
...
| Preferences/app state | `FileAppStateStore`, `UserAppStateStore`, `PreferenceConsumerMatrix`, session state, company UI preference/state tables, production `CompanyTableStateBinder` | mixed; company money/date and table/divider display state are H2, while theme, stage decoration, top-level window geometry, and shell dividers are user-machine state | production startup loads shell preferences before composition; geometry/dividers are read and written only when **Remember window state** is enabled; connected database path remains read-only factual runtime authority; compatibility `defaultPrivilege`/`defaultReopenScope` values are disabled and are not authorization/policy consumers | preserve the executable consumer matrix; database switching remains owned by `DatabaseSessionController`; no preference editor may change active database authority or present a compatibility value as security |
...
| Global command capabilities | transient `AppPanel.commandCapabilities()` plus `GlobalCommandRegistry` definitions | not applicable; capability state is UI runtime state | no command capability, enabled state, or shortcut registry is persisted; a command delegates to its owning service/persistence boundary only after declared support | keep runtime capability state derived from the active panel/Administration tab; never introduce a parallel persistence path for command availability |
...
- Bank-statement import alone does not create canonical accounting effects. P16-S8 permits one explicit reviewed-row acceptance through `ReviewedStatementAcceptanceService` into canonical `Txn`/`TxnSplit`; P05-C8 presents canonical configured-bank split lines under **Bank Transactions → Ledger Activity** while imported rows remain evidence under **Statement Review**. Configured-bank identity requires the persisted `CompanyBankAccount` company/account link, and the exact `TxnSplit` remains authoritative for signed amount and cleared state/date. No second bank-transaction table or sidecar is introduced.
- P15-S5-C2 through C10 SCLX import writes one target company graph in one caller-owned JPA transaction. P16-C6 introduced nondestructive chart/fund reuse; P16-C9 makes the merge identity-aware across the graph: unrelated target operational history is retained, `IDENTICAL` records are reused, `NEW` records are added, and each `CONFLICT` requires a revalidated per-record target/source choice where the source writer is safe. Target settings/chart metadata remain preserved. Accounts and funds resolve before normalized budgets and dependent canonical transactions; activities, counterparties, and merchants resolve before transaction relationships; corrections follow transaction creation; fixed assets and completed runs follow their account, fund, and transaction references; inventory history follows its dependencies; banking/reconciliation follows chart, transaction, and statement creation; authoritative period-close and factual audit history are restored after the ledger graph exists. Concrete ownership, mapping, closed-period, finalized-reconciliation, range-overlap, reference, and business-key conflicts remain blocking.
