...
Status: P00 inventory of current main, updated through P15-S5-C8 selected-company SCLX period-close import. This document identifies duplicate authority risks, non-H2 stores, and migration hazards before later phases choose canonical models.
...
- P15-S5-C2 through C8 SCLX import writes one empty target company graph in one caller-owned JPA transaction. Accounts and funds are created before normalized budgets and dependent canonical transactions; activities, counterparties, and merchants are created before transaction relationships; fixed assets and completed runs are created after their account, fund, and transaction references exist; inventory items and movement history follow their account, fund, item, and optional transaction dependencies; bank configuration and reviewed statement facts follow chart and transaction creation; transaction-line clearance and reconciliation follow their statement/ledger dependencies; authoritative period-close ranges and factual events are restored after the ledger graph exists.
