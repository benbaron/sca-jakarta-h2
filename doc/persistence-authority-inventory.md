...
Status: P00 inventory of current main, updated through P15-S5-C7 selected-company SCLX banking and reconciliation import. This document identifies duplicate authority risks, non-H2 stores, and migration hazards before later phases choose canonical models.
...
- P15-S5-C2 through C7 SCLX import writes one empty target company graph in one caller-owned JPA transaction. Accounts and funds are created before normalized budgets and dependent canonical transactions; activities, counterparties, and merchants are created before transaction relationships; fixed assets and completed runs are created after their account, fund, and transaction references exist; inventory items and movement history follow their account, fund, item, and optional transaction dependencies; bank configuration and reviewed statement facts follow chart and transaction creation, transaction-line clearance follows statement facts, and reconciliation sessions/matches are written last after every referenced bank account, statement line, and transaction split exists.
