...
The old `open_item_snapshot` table is compatibility data and is not a current Dashboard authority. P22-S5 restores Open Items from `txn_supplemental_line` lifecycle allocations through the same `SupplementalOpenItemQueryService` used by Report Library. The card shows the six authoritative domains—Receivables, Payables, Prepaid Expenses, Deferred Revenue, Other Assets, and Other Liabilities. For the selected period end, the Dashboard counts authoritative items with a positive open balance and sums only those positive balances by supplemental kind. Closed items remain authoritative but contribute zero count/amount; over-applied or inconsistent diagnostic rows are visible in the corresponding reports and are not silently folded into Dashboard totals. `REVERSED` and future transactions contribute nothing. When a company has no lifecycle-aware authoritative item rows, the card remains explicitly unavailable; retained `open_item_snapshot` rows do not make it available.
