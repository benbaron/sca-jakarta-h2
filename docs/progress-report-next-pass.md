...
## 50) Tightened next-steps prompt (supersedes #49)

> Continue from `docs/progress-report-next-pass.md` and deliver **only** this bounded slice:
> 
> **Scope (required):**
> 1) Add `LedgerQueryService` contract tests for:
>    - `listRecent(int)` ordering (`txnDate DESC`, then `id DESC`) and max-row limit,
>    - `journalForTxn(Long)` DR/CR mapping by `NormalBalance` and signed amount,
>    - stable row-shape assertions (required fields non-null/expected defaults).
> 2) Extract query access behind a repository interface (no behavior changes), and keep `LedgerQueryService` as orchestration/translation only.
> 3) Add/adjust tests so service behavior is validated against repository outputs with deterministic fixtures.
>
> **Out of scope (do not do):**
> - No container-specific Maven/network workaround engineering.
> - No unrelated UI refactors.
> - No schema/migration changes unless strictly required by failing tests.
>
